#!/usr/bin/env bash
#
# serve.sh — serve the built Quartz site (repo's "public" dir) on a fixed
# Tailscale-only address. Refuses to start on a wildcard/loopback bind
# address so the site is never accidentally exposed beyond Tailscale.
#
# Env overrides:
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Serving '$REPO_DIR/public' on tcp://${BIND_HOST}:${PORT}"
exec node ./node_modules/.bin/serve -l "tcp://${BIND_HOST}:${PORT}" public
