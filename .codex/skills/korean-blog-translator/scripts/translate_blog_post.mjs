...
async function run(argv) {
  ...
  if (args.provider === "solar" && !process.env.UPSTAGE_API_KEY) {
    throw new WorkflowError("UPSTAGE_API_KEY is required for --provider solar")
  }

  const source = readFileSync(args.source, "utf8")
  const parsed = splitFrontmatter(source, args.source)
  const hash = sourceHash(source)
  const translations = []

  for (const locale of args.locales) {
    const targetPath = join(args.outDir, locale, basename(args.source))
    ...
    const translatedBody = await translateBody(parsed.body, locale, args.provider)
    const targetFrontmatter = buildFrontmatter(parsed.frontmatter, locale, hash, args.source)
    mkdirSync(join(args.outDir, locale), { recursive: true })
