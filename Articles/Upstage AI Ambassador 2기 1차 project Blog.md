...
이번 과제를 계기로, 제가 가장 자주 사용하던 논문 작성 도구인 **Overleaf 를 포크 (fork)** 하여, AI 기반 참고문헌 관리 패널을 추가한 **My Awesome RA**를 구현했습니다.
...
Evidence Panel 을 위해 **Reference Library**를 통해 참고문헌을 관리합니다. PDF 업로드, 인덱싱 상태 확인, 임베딩 시도가 가능하며, 업로드된 PDF 는 Upstage Document Parse API 를 통해 자동으로 인덱싱됩니다.
...
현재 작성 중인 문서의 맥락과 관련 인용을 함께 고려하여 답변을 생성하므로, "AI 가 지어낸 답변 " 이 아니라 **실제 참고문헌에 기반한 연구 보조**가 가능합니다.
