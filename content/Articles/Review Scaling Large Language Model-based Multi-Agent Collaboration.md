...
### 1. Critic 의 역할이 정말 동일한가?

Review 를 하면서, Critic 의 역할에 대한 질문을 받아서 궁금증이 생겼다. 이해한대로라면 단일 critic 의 경우 edge 로써 두 노드만 바라보게 되는데 그렇다면 여러 critic 들을 둬야 할텐데 critic 마다의 역할이 다른지 궁금했다. 실제 [GitHub - OpenBMB/ChatDev at macnet](https://github.com/OpenBMB/ChatDev/tree/macnet) 로 가서 구현을 확인해보니 role 이 critic 마다 다르게 구현 된 것을 확인 할 수 있었다.
