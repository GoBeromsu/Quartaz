...
| 앱 | CLI 명령 | 출력 포맷 |
|-----|---------|----------|
| Zotero | `zt collections` | `ZK3ATEUT \| 00. Inbox` (key \| name) |
| Eagle | `eg folder:list` | `M8RRO78FRS1VZ \| Resources/Meme` (id \| path) |
| DEVONthink | `dt groups` | `/11. Project/창업` (path only) |
...
| 앱 | CLI 명령 | 특이사항 |
|-----|---------|---------|
| Zotero | `zt import` | 실패 시 `zt create:item` + `zt attach` 2 단계 fallback |
| Eagle | `eg import:path` | 파일명에서 따옴표 제거 |
| DEVONthink | `dt add --db "01. Personal"` | 고정 DB |
...
[[Process Manager 2 (PM2)|PM2]] 를 사용해서 언제든 꺼지지 않도록 작성하였습니다.
